...
//				foreach ($this->upload_infos as $upload_info)
		...
		public static function delete_default_local_files($name_hash, $delete_temp, $delete_log_files = true)
		{
			...
			$legacy_installer_temppath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$name_hash}_{$global->get_legacy_installer_filename()}");						
			...
			$legacy_installer_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}_{$global->get_legacy_installer_filename()}");			
			...
			if ($delete_temp)
			{
				...
				@chmod($legacy_installer_temppath, 0644);
			...
			@chmod($legacy_installer_filepath, 0644);
			...
			if ($delete_temp)
			{
				...
				@unlink($legacy_installer_temppath);
			...
			@unlink($installer_filepath);																								
			@unlink($legacy_installer_filepath);
		...
		public function get_legacy_installer_filename()
		{			
			$global = DUP_PRO_Global_Entity::get_instance();			
			
			return "{$this->NameHash}_{$global->get_legacy_installer_filename()}";
		}
		...
		public function build_integrity_check()
		{
			...
			//------------------------
			//LEGACY INSTALLER CHECK: 
			$legacy_exe_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP) . "/{$this->NameHash}_{$global->get_legacy_installer_filename()}";
			$legacy_exe_done_txt = DUP_PRO_Util::TailFile($legacy_exe_filepath, 10);
			
			if (!strstr($legacy_exe_done_txt, 'DUPLICATOR_PRO_INSTALLER_EOF') && !$this->build_progress->failed)
			{
				$this->build_progress->failed = true;
				$this->update();
				$this->set_status(DUP_PRO_PackageStatus::ERROR);
				DUP_PRO_Log::Error("ERROR: Legacy installer file not complete.  The end of file marker was not found.  Please try to re-create the package.", '', false);
				return;
			}
