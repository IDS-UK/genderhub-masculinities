...
function learndash_admin_tabs() {
	...
	$admin_tabs_on_page = array(
			...
			'admin_page_learndash_support' => array( 135, 136, 140, 150 ),			
...
function add_essays_data_columns( $cols ) {
	...
	$essay_columns = array(
		...
		'course' 	=> 	sprintf( _x( 'Assigned %s', 'Assigned Course Label', 'learndash' ), LearnDash_Custom_Label::get_label( 'Course' ) ),
		'lesson' 	=> 	sprintf( _x( 'Assigned %s', 'Assigned Lesson', 'learndash' ), LearnDash_Custom_Label::get_label( 'Lesson' ) ),
		'quiz'		=> 	sprintf( _x( 'Assigned %s', 'Assigned Quiz', 'learndash' ), LearnDash_Custom_Label::get_label( 'quiz' ) ),
	...
	if ( is_admin() AND $pagenow == 'edit.php'  AND isset( $_GET['post_type'] ) AND ( $_GET['post_type'] == 'sfwd-lessons' OR $_GET['post_type'] == 'sfwd-topic' OR $_GET['post_type'] == 'sfwd-quiz' OR $_GET['post_type'] == 'sfwd-assignment' OR $_GET['post_type'] == 'sfwd-essays' ) ) {
		...
		if ( $lazy_load == true ) {
			...
			if ( $lazy_load == true ) {
				...
				$query_options['posts_per_page'] 	= 	apply_filters('learndash_element_lazy_load_per_page', LEARNDASH_LMS_DEFAULT_LAZY_LOAD_PER_PAGE, $_GET['post_type']);
			}
		}

		$query_options = apply_filters( 'learndash_course_post_options_filter', $query_options, $_GET['post_type'] );		
		...
		if ( $_GET['post_type'] == 'sfwd-topic' OR $_GET['post_type'] == 'sfwd-assignment' OR $_GET['post_type'] == 'sfwd-essays' ) {
			...
			$query_options = apply_filters( 'learndash_lesson_post_options_filter', $query_options, $_GET['post_type'] );		
...
function course_table_filter( $query ) {
	...
	if ( ( is_admin() ) && ( $query->is_main_query() ) && ( $pagenow == 'edit.php' ) && ( !empty( $typenow ) ) ) {

		if ( ( isset( $_GET['course_id'] ) ) && ( !empty( $_GET['course_id'] ) ) && ( $typenow == 'sfwd-lessons' || $typenow == 'sfwd-topic' || $typenow == 'sfwd-quiz' || $typenow == 'sfwd-assignment' || $typenow == 'sfwd-essays' ) ) {
		...
		<div class='sfwd_options_wrapper sfwd_settings_left'>
			<div class='postbox ' id='sfwd-course_metabox'>
				...
				echo '<b>' . __( 'Shortcode Options', 'learndash' ) . '</b>
					...
					<p><b>[ld_profile]</b></p><p>' . sprintf( _x( 'Displays user\'s enrolled %s, %s progress, %s scores, and achieved certificates.', 'placeholder: courses, course, quiz', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'courses' ), LearnDash_Custom_Label::label_to_lower( 'course' ), LearnDash_Custom_Label::label_to_lower( 'quiz' ) ) . '</p>
					...
					<br>
                    <p><b>[student]</b></p><p>' . sprintf( _x( 'This shortcode shows the content if the user is enrolled in the %s. Example usage: <strong>[student]</strong>Welcome Student!<strong>[/student]</strong>', 'placeholders: course', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'course' ) ) . '</p>
					<br>
