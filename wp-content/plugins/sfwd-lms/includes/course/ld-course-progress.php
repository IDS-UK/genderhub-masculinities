...
//			$can_attempt_again = learndash_can_attempt_again( $user_id, $quiz_id );
//			error_log('can_attempt_again['. $can_attempt_again .']');
//
//			if ( $can_attempt_again ) {
...
//			}
			...
			$link = learndash_next_lesson_quiz( true, $user_id, $lesson_id );
...
function learndash_next_lesson_quiz( $url = true, $user_id = null, $lesson_id = null, $exclude = array() ) {
	...
	if ( empty( $return ) ) {
		// Why WHY is this here? We should not be mixing logic for quizzes and topics. Bad form!
		// I realize the goal here is if all the lesson quizzes are complete then we then mark the parent lesson complete.
		// But AGAIN this is mixing logic into this function whose goal is simply to determin the next quiz URL to
		// direct the user to. damit.
