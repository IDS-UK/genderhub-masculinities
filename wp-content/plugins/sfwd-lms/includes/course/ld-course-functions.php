...
function ld_lesson_access_from( $lesson_id, $user_id ) {
	...
	if ( $visible_after > 0 ) {
		...
		$lesson_access_from = $couses_access_from + abs($visible_after) * 24 * 60 * 60;
		...
		if (!empty($visible_after_specific_date)) {
			...
			//error_log('specific_date['. $specific_date .'] specific_date_YYMMDD['. $specific_date_YYMMDD .']');

			//$time = time();
			//$time += $seconds_offset;
			$time = current_time( 'timestamp' );
			
			//$time_YYMMDD = date('Y-m-d H:i:s', $time);
			//error_log('time['. $specific_date .'] time_YYMMDD['. $time_YYMMDD .']');
