...
function learndash_course_expire_status_shortcode( $atts, $content ) {
	...
	if ( ( !empty( $atts['course_id'] ) ) && ( !empty( $atts['user_id'] ) ) ) {
		if ( sfwd_lms_has_access( $atts['user_id'], $atts['course_id'] ) ) {
			...
			$couses_access_from = ld_course_access_from( $atts['course_id'], $atts['user_id'] );			
			if ( empty( $couses_access_from ) ) {
				$couses_access_from = learndash_user_group_enrolled_to_course_from( $atts['user_id'], $atts['course_id'] );
			}
