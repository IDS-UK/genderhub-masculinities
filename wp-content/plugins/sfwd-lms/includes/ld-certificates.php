	...
	if ( ( $post instanceof WP_Post ) && ( $post->post_type == 'sfwd-quiz' ) ) {
		...
		if ( is_array( $meta ) && ! empty( $meta ) ) {
			...
			if ( (isset( $meta['sfwd-quiz_certificate'] )) && ( ! empty( $meta['sfwd-quiz_certificate'] ) ) ) {
				...
				$certificateLink = apply_filters('learndash_certificate_details_link', $certificateLink, $certificate_post, $post->ID, $user_id);
...
function learndash_get_course_certificate_link( $course_id, $user_id = null ) {
	...
	return apply_filters('learndash_course_certificate_link', $url, $course_id, $user_id);
}
