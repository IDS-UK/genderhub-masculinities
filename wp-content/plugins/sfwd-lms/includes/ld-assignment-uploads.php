		...
		?>
			<script type="text/javascript">
				jQuery(document).ready(function() {
					jQuery('<option>').val('approve').text('<?php echo $approve_text ?>').appendTo("select[name='action']");
					jQuery('<option>').val('approve').text('<?php echo $approve_text ?>').appendTo("select[name='action2']");
				});
			</script>
		<?php
	}
...
function learndash_restrict_assignment_listings( $query ) {
	...
	if ( ( is_admin() ) && ( $query->is_main_query() ) && ( $pagenow == 'edit.php' ) && ( !empty( $typenow ) ) && ( $typenow == 'sfwd-assignment' ) ) {
		if ( !current_user_can( 'manage_options' ) ) {
